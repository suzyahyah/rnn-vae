...
class TextDataset(Dataset):
    def __init__(self, fn="", model="vae", nwords=10000):
        self.nwords = nwords
        self.model = model
    ...
    def __getitem__(self, ix):
        ...
        sent = self.data[ix]
        sent = [self.w2ix[w] for w in sent.split()]
    ...
    def make_ix_dicts(self, all_words):
        ...
        if len(c_all_words) > self.nwords:
            vocab_words = c_all_words.most_common(self.nwords)
        else:
...
def collate_fn(data):
    ...
    return train_seq_lens, train_seq, eos_seq
